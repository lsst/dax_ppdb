...
if [ -z "$SERVICE_ACCOUNT" ]; then
  echo "SERVICE_ACCOUNT is not set. Please set it to your Google Cloud service account."
  exit 1
fi
...
gcloud functions deploy trigger_stage_chunk \
  ...
  --set-env-vars "PROJECT_ID=${PROJECT_ID},REGION=${REGION},DATAFLOW_TEMPLATE_PATH=${DATAFLOW_TEMPLATE_PATH},SERVICE_ACCOUNT=${SERVICE_ACCOUNT},DATASET_ID=${DATASET_ID},TEMP_LOCATION=${TEMP_LOCATION}" \
  --region=${REGION} \
  --source=. \
