...
class PpdbConfig(BaseModel):
    ...
    def from_uri(cls, uri: ResourcePathExpression) -> PpdbConfig:
        ...
        class_name = config_object.get("cls")
        if not class_name:
            raise LookupError("YAML configuration file does not have `cls` key")
        klass = doImportType(class_name)
        if not issubclass(klass, PpdbConfig):
            raise TypeError(f"Class {klass!r} is not a subclass of PpdbConfig")
        return klass(**config_object)
