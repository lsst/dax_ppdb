...
class UpdateRecordExpanderTestCase(unittest.TestCase):
    ...
    def test_update_records_all(self) -> None:
        ...
        # Verify that the expanded records match the expected order and
        # contents from the input records.
        expected = [
            # From ApdbReassignDiaSourceToDiaObjectRecord (update_order=0)
            ("DiaSource", (100001,), "diaObjectId", 300001, 0, test_update_time_ns),
            # From ApdbReassignDiaSourceToSSObjectRecord (update_order=1)
            ("DiaSource", (100002,), "ssObjectId", 2001, 1, test_update_time_ns),
            ("DiaSource", (100002,), "ssObjectReassocTimeMjdTai", test_mjd_tai, 1, test_update_time_ns),
            # From ApdbWithdrawDiaSourceRecord (update_order=2)
            ("DiaSource", (100003,), "timeWithdrawnMjdTai", test_mjd_tai, 2, test_update_time_ns),
            # From ApdbWithdrawDiaForcedSourceRecord (update_order=3)
            (
                "DiaForcedSource",
                (200001, 12345, 42),
                "timeWithdrawnMjdTai",
            ...
            # From ApdbCloseDiaObjectValidityRecord (update_order=4)
            ("DiaObject", (200001,), "validityEndMjdTai", test_mjd_tai, 4, test_update_time_ns),
            ("DiaObject", (200001,), "nDiaSources", 5, 4, test_update_time_ns),
            # From ApdbUpdateNDiaSourcesRecord (update_order=5)
            ("DiaObject", (200002,), "nDiaSources", 10, 5, test_update_time_ns),
            # From duplicate ApdbReassignDiaSourceToDiaObjectRecord
            ("DiaSource", (100001,), "diaObjectId", 400001, 0, test_update_time_ns + 1000000000),
            # From duplicate ApdbUpdateNDiaSourcesRecord (earlier timestamp)
            ("DiaObject", (200002,), "nDiaSources", 8, 5, test_update_time_ns - 1000000000),
        ]

        for i, (table_name, record_id, field_name, field_value, update_order, update_time_ns) in enumerate(
            expected
        ):
            with self.subTest(i=i, field_name=field_name):
                self.assertEqual(expanded[i].table_name, table_name)
                self.assertEqual(expanded[i].record_id, record_id)
                self.assertEqual(expanded[i].field_name, field_name)
                self.assertEqual(expanded[i].field_value, field_value)
                self.assertEqual(expanded[i].update_order, update_order)
